<script>
    ...
    async function captureFrame() {
        ...
        try {
            const resp = await fetch(`${API_BASE}/api/gallery_thumbs`, {
                method: 'POST',
    ...
    <div class="max-w-6xl mx-auto py-6 px-4 border border-white">
        ...
        {:else}
            <div class="bg-black aspect-video w-full border border-white">
                <video bind:this={videoEl} src={videos[currentIndex] ? `${API_BASE}/videos/${videos[currentIndex].name}` : ''} controls autoplay playsinline class="w-full h-full object-contain"></video>
            </div>
